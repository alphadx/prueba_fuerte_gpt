...
def test_employees_forbid_cajero_listing() -> None:
    response = client.get("/employees", headers=_auth_header(roles=["cajero"]))
    assert response.status_code == 403


def test_employees_crud_flow_for_rrhh() -> None:
    rrhh_headers = _auth_header(roles=["rrhh"])
    admin_headers = _auth_header(roles=["admin"])

    create_response = client.post(
        "/employees",
        json={"employee_code": "E-001", "full_name": "Ana Pérez", "role": "bodega", "is_active": True},
        headers=rrhh_headers,
    )
    assert create_response.status_code == 201
    created = create_response.json()

    list_response = client.get("/employees", headers=rrhh_headers)
    assert list_response.status_code == 200
    assert any(item["id"] == created["id"] for item in list_response.json()["items"])

    patch_response = client.patch(
        f"/employees/{created['id']}",
        json={"role": "cajero", "is_active": False},
        headers=rrhh_headers,
    )
    assert patch_response.status_code == 200
    assert patch_response.json()["role"] == "cajero"

    forbidden_delete = client.delete(f"/employees/{created['id']}", headers=rrhh_headers)
    assert forbidden_delete.status_code == 403

    allowed_delete = client.delete(f"/employees/{created['id']}", headers=admin_headers)
    assert allowed_delete.status_code == 204


def test_employees_reject_duplicate_code() -> None:
    ...
    second = client.post(
        "/employees",
        json={"employee_code": "E-001", "full_name": "Juan Pérez", "role": "rrhh", "is_active": True},
        headers=headers,
    )
